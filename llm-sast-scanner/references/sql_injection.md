...
- ORM builders **on value arguments**: `.filter(col=val)`, `.where(col: val)`, `.findOne()`, `.findUnique()` (but a user-controlled *field name* passed to `.values()`/`.order_by()`/`.annotate()` is an identifier sink — see #5)
