...
> **Shortcut:** `llm-sast-scanner-full-scan-loop <dir>` performs a partitioned version of this fan-out natively (every lens x 3 partitions, one subagent each); `llm-sast-scanner-convergence-loop <dir>` performs the unpartitioned one-subagent-per-lens fan-out. The steps are spelled out here so the orchestrator can drive them directly when preferred.
