---
name: llm-sast-scanner-convergence-loop
description: >
  Convergence-driven, exhaustive line-by-line security audit wrapper around the llm-sast-scanner skill.
  Invoke explicitly as "llm-sast-scanner-convergence-loop <dir> [mode=parallel|single]" where <dir> is the target
...
```
llm-sast-scanner-convergence-loop <dir> [mode=parallel|single] [adv=critical,high,medium] [lens=<lens>] [new-scan]
```
...
> lens=<their lens>`). They MUST NOT re-invoke this `llm-sast-scanner-convergence-loop` wrapper, or it would
> fan out again.
